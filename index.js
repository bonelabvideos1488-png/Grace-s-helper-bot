...
// ============================================================
// 5. АВТОШТРАФ ЗА "КРОКУС" (ВСЕ ЯЗЫКИ, ВАРИАНТЫ, СИМВОЛЫ)
// ============================================================
...
bot.on('message', (msg) => {
    ...
    // Удаляем все знаки препинания и пробелы для проверки "перед классными"
    ...
    // 1. Основные варианты (русский, английский, транслит, цифры)
    const krokusRegex = new RegExp(
        '(?:' +
        // Русский: крокус
        '[кkк][рpр][оo0о][кkк][уyу][сcс]' +
        '|' +
        // Английский: crocus
        '[cсc][рpр][оo0о][cсc][уyу][sс]' +
        '|' +
        // Транслит: krokus
        '[kкk][рpр][оo0о][kкk][уyу][sс]' +
        '|' +
        // Смешанный: cрокус, kрокус, крокуc
        '[cсckкk][рpр][оo0о][cсckкk][уyу][sсc]' +
        ')',
        'i'
    );

    // 2. Варианты с символами между буквами (к.р.о.к.у.с, к-р-о-к-у-с, к_р_о_к_у_с)
    const krokusWithSymbolsRegex = new RegExp(
        '[кkк]\\W*[рpр]\\W*[оo0о]\\W*[кkк]\\W*[уyу]\\W*[сcс]',
        'i'
    );

    // 3. Арабский: كروكوس
    const arabicRegex = /كروكوس/i;

    // 4. Урду (пакистанский): کروکوس
    const urduRegex = /کروکوس/i;

    // 5. Французский / Итальянский / Испанский (транслит)
    const latinRegex = /[cс]\\W*[рpр]\\W*[оo0о]\\W*[сc]\\W*[уyу]\\W*[sс]/i;

    // 6. Проверка "перед классными"
    const beforeClassRegex = /крокус\s*классн[ыо]й?|крокус\s*классн[ыо]е|крокус\s*классн[ыо]|crocus\s*class/i;

    // Проверяем, есть ли совпадение
    const found = 
        krokusRegex.test(text) ||
        krokusWithSymbolsRegex.test(text) ||
        ...
        /к\W*р\W*о\W*к\W*у\W*с/i.test(text) ||
        /c\W*р\W*о\W*c\W*у\W*s/i.test(text) ||
        /k\W*р\W*о\W*k\W*у\W*s/i.test(text);
